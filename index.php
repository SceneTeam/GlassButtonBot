<?php
ob_start();
define('API_KEY','259466540:AAFW_Ojgdobfk5_KGZOcNWM_T0Dktr9ijwc');
$the_admin_id = 0;
file_put_contents("count",file_get_contents("count")+1);
$ad_btn='آموزش های رایگان ساخت ربات های تلگرام';
$ad_url='http://telegram.me/EleGanceTM';
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@GlassButtonBot getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
<pre>@GroupButtonBot d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
➖➖➖➖➖➖
قبل از هر کاری اول باید بدانید این via GlassButton چیست !
...
<code>@GlassButtonBot getlist-XXXXX </code>
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@GlassButtonBot getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@GlassButtonBot getlist-','',$code));
...
https://telegram.me/EleGanceTM
به ربات ما سر بزنید
https://telegram.me/GlaasButtonBot
                            "
                        ]);
                    }else{
                        $newList = [];
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@GlassButtonBot getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@GlassButtonBot getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                    ]);
//                    makeHTTPRequest('sendMessage',[
//                        'text'=>'@GlassButtonBot getbanner-'.getCurrentMakeID($userID),
//                        'chat_id'=>$userID
//                    ]);
//                }break;
                case '7':{
                ...
                case '9':{
                    if(is_valid_url($userTEXT)) {
                        ...
                        makeHTTPRequest('sendMessage',[
                            'text'=>"شناسه کانال را بدهید. مانند @EleGanceTM\n\nاگر کانال شناسه ندارد کامند /skipstep  را بزنید\nجهت انصراف ، کامند /cancel را وارد کنید",
                            'chat_id'=>$userID
                        ]);
                    }else{
                        makeHTTPRequest('sendMessage', [
        ...
        default:{
            $links = [
                'EleGanceTM'=>'https://telegram.me/EleGanceTM',
                'TGMassengerBot'=>'https://telegram.me/TGMassengerBot',
                'EleGanceRoBot'=>'https://telegram.me/EleGanceRoBot',
                ...
                'reply_markup'=>json_encode([
                    'inline_keyboard'=>[
                      ...
                      [['text'=>'به رایگان در تلگـ ـلیست کانال خود را ثبت کنید','url'=>'http://teleglist.com/?bot']],
                        ...
                        [['text'=>'تماس با توسعه دهنده','url'=>'https://telegram.me/TGMassengerRoBot']],
                        [['text'=>'ربات پیام رسان ساز','url'=>'https://telegram.me/TGMassengerBot']]
                    ]
                ])
            ]));
        }
